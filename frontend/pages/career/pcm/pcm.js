...
import {
  BookOpen, GraduationCap, Briefcase, ChartBar, Rocket, Microscope, School,
  Award, Building2, Plane, Shield, Ship, Code2, Brain, Calculator, X, ExternalLink, Loader2
} from "lucide-react";
// Import data and the new generation function
import { examOptions, degreeOptions, careerOptions, generateContentForOption, getFallbackContent } from "./data"; // Adjust path if necessary

const iconComponents = {
  BookOpen, GraduationCap, Briefcase, ChartBar, Rocket, Microscope, School,
  Award, Building2, Plane, Shield, Ship, Code2, Brain, Calculator
};
...
const ContentModal = ({ isOpen, onClose, title, content, isLoading, optionPath, onNavigate }) => {
  ...
  React.useEffect(() => {
    if (isOpen) {
      ...
      // Start with opening animation, then move to open state
  ...
  const formattedContent = () => {
    ...
    // If typewriter effect is complete or we want to skip it, show the full content
    const contentToDisplay = typewriterComplete ? content : typedContent;

    // Split by paragraphs and add proper HTML structure
    const paragraphs = contentToDisplay.split('\n').filter(p => p.trim() !== '');

    if (paragraphs.length === 0) return null;

    // Extract first paragraph as heading/intro if it's short enough
    ...
    return (
      <div className="space-y-4">
        ...
        {/* Show "continue reading" button only when done typing */}
  ...
  // Dynamic animation classes based on animation state
  ...
  return (
    <div className={`fixed inset-0 z-50 flex justify-center items-center p-4 ${backdropClasses}`}>
      ...
      >
        ...
        {/* Footer with External Link - visible only when typing is complete */}
...
const PCM = () => {
  ...
  const [selectedOption, setSelectedOption] = useState(null); // Store the whole option
  const [isLoading, setIsLoading] = useState(false);
  const [hasError, setHasError] = useState(false);

  // Function to handle opening external links or internal routes
  const handleNavigation = (path) => {
    if (!path) return;
    if (path.startsWith("http")) {
      window.open(path, "_blank", "noopener,noreferrer"); // Added security attributes
    } else {
      router.push(path);
    }
  };
...
const OptionCard = ({ option, onClick }) => {
  ...
  // Construct safe color class
  let colorClass = 'text-gray-500'; // Default fallback
  ...
  // Determine which options array to display based on the toggle state
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            <div className="space-y-4">
              <div className="flex items-center mb-2">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-gray-800">About PCM</h2>
              </div>
              <p className="text-gray-600 leading-relaxed text-base md:text-lg">
                The PCM stream (Physics, Chemistry, Mathematics) is a popular
                academic path for students interested in science, technology,
                and innovation. It provides a strong foundation in analytical
                thinking, problem-solving, and logical reasoning. PCM is ideal
                for students who enjoy mathematics and science subjects and are
                interested in pursuing careers in engineering, architecture,
                data science, research, and other technical domains.
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="space-y-2">
                {[
                    "Physics", "Chemistry", "Mathematics", "English", "Optional (e.g., CS, PE)"
                ].map((subject, idx) => (
        ...
        <div className="flex justify-center my-8 md:my-10">
          <div className="relative bg-gray-100 rounded-full flex p-1 w-auto max-w-md shadow-md overflow-hidden">
            <div
              ...
              style={{
                width: `${100 / 3}%`, // Dynamic width based on number of options
      ...
      <ContentModal
        ...
        isLoading={isLoading}
        optionPath={selectedOption?.path} // Pass the path for the link button
