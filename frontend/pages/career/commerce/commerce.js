...
const Commerce = () => {
  ...
  const handleShowDetails = async (option) => {
    ...
    try {
      setTimeout(async () => {
        try {
          const generatedContent = await generateContentForOption(option);
          setModalContent(generatedContent);
        } catch (error) {
          console.error("Error generating content:", error);
          const fallbackContent = getFallbackContent(option);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            <div className="space-y-4">
              <div className="flex items-center mb-2">
                <Calculator className="w-7 h-7 text-blue-500 mr-3 flex-shrink-0" />
                <h2 className="text-2xl md:text-3xl font-bold text-gray-800">
                  About Commerce
                </h2>
              </div>
              <p className="text-gray-600 leading-relaxed text-base md:text-lg">
                The Commerce stream provides a strong foundation in business,
                finance, and management studies. It prepares students for
                careers in accounting, finance, banking, and business
                management. Commerce is ideal for students interested in
                business operations, financial analysis, market research, and
                entrepreneurship. This stream opens doors to careers in
                corporate finance, banking, accounting, business management, and
                entrepreneurship.
              </p>
            </div>
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="space-y-2">
                {[
                  ...
                  "Economics",
                  "Mathematics",
                  "Optional(Computer Science, Informatics Practices, etc)"
                ].map((subject, idx) => (
